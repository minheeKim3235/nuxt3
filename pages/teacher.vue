<template>
  <section class="content">
    <div class="cont3">
      ...
      <div class="special_teacher_list" v-if="isList">
        <ColumnList :items="teacherList" :columns="2" :gap="6" :addClass="setBrand">
          <template #default="{ item }">
            <PubTeacherList :item="item" @on-click="setActiveItem(item)"/>
          </template>
          <template #append v-if="setBrand == 'HPnormal'">
            <li>
              <div class="box2">
                <strong>입시전문가</strong>
                <p>MMI, 서울대 심층면접대비<br />문제출제, 검수, 강의, 면접 대비</p>
              </div>
            </li>
            <li>
              <div class="box2">
                <strong>탁월한 입시감각</strong>
                <p>수시·정시의 해답을 제시해<br />입시를 성공으로 이끄는<br />비교불가 전문가 집단</p>
              </div>
            </li>
          </template>
        </ColumnList>
    ...
    <SelectBrand :items="brands" v-model="setBrand" />
  </section>
</template>
...
// data
const brands = ref(['청솔', '청솔네오', '강하']);
const setBrand = ref('HPnormal')
